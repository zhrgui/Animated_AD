...
def process_video_every_frame(decord_vr, start=None, end=None):
    """
    Input: decord video reader file
    Output: tuple containing:
        - list of all numpy frames between start and end times
    """
    duration, local_fps = len(decord_vr), float(decord_vr.get_avg_fps())
    ...
    if start is not None and end is not None:
        ...
        frame_id_list_all = np.arange(start_frame, end_frame + 1, dtype=int)
    ...
    try:
        video_data_all = decord_vr.get_batch(frame_id_list_all).numpy()
    except:
        video_data_all = decord_vr.get_batch(frame_id_list_all).asnumpy()

    images_all = [f.numpy() if isinstance(f, torch.Tensor) else f for f in video_data_all]
    return images_all
