...
def main(args):
    ...
    print(f"Processing {args.pred_path}")
    ...
    for _, row in tqdm(pred_df.iterrows(), total=len(pred_df)):
        ...
        user_prompt = get_user_prompt(prompt_idx=args.prompt_idx, verb_list=verb_list, text_pred=text_pred, duration=round(row['end_']-row['start_'], 2), ad_speed=ad_speed)
    ...
    output_df.to_csv(os.path.join(args.output_dir, f"stage2_llama3_{args.prompt_idx}_" + os.path.basename(args.pred_path)))
