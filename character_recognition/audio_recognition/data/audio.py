...
def load_wav(path, fr=0, to=10000, sample_rate=16000):
    ...
    # If stereo (shape (k, 2)), convert to mono by averaging the channels
    if wav.ndim == 2:
...
def wav2filterbanks(wav, mel_basis=None):
    ...
    # Follow the input, not the default cuda device: under DDP the batch lives on
    # cuda:<rank> and a window built on cuda:0 would fail on every other rank.
    device = wav.device
    ...
    if mel_basis is None:
        # Build a Mel filter. Keyword arguments: librosa made them keyword-only
        # in 0.10, and they were already named this way before that.
